...
dag_params = {
    'dag_id': "shustikov_dag",  # Dag id
    'description': 'Процесс подгружает дополнительный файл, пересчитывает метрики, сохраняет графики',
    'start_date': datetime(2023, 1, 1),
    'schedule_interval': '@once',  # Интервал сделан по запросу с целью не засорять яндекс диск
    'catchup': False
}

# Для проверки работоспособности оставляю ссылку на сам DAG: https://airflow-da.lab.karpov.courses/tree?dag_id=shustikov_dag_v3
# Так же оставляю данные на расшариный каталог на Яндекс Диске куда сохраняется график(для примера там оставлен график за 2023-02-08): https://disk.yandex.ru/d/6R2Bs2cMgRncmw

# Перечсисляем константные переменные
additional_filepath = 'https://disk.yandex.ru/d/5Kxrz02m3IBUwQ'
...
def _save_updated_metrics_csv(additional_filepath, updated_metrics_filepath, sep=','): # Функция подсчета метрик и сохрания их в файл
    import pandas as pd
    import warnings
...
def _resave_images(updated_metrics_filepath, metrics_image_filepath): # Фунция считывания файла с метриками и 
                                                                      # сохраниение графиков в формате .png
    import pandas as pd
    import matplotlib.pyplot as plt
    import seaborn as sns
...
def _remove_updated_metrics_from_local(updated_metrics_filepath): # Удаление файла с метриками с локального диска
    import os
    os.remove(updated_metrics_filepath)
    return True

def create_folder(path_save_file):  # Функция создания каталога на яндекс диске
    import requests
    requests.put(f'{URL}?path={path_save_file}', headers=headers)

def upload_file(loadfile, savefile, replace=True): # Функция копирования (загрузки) файла на Яндекс Диск
    import requests
...
def _save_file_in_yandex_disk(path_save_file,metrics_image_filepath): # Функция вызывающая функции для работы с яндекс диском
    create_folder(path_save_file)
    upload_file(f'{metrics_image_filepath}',f'{path_save_file}/{metrics_image_filepath}')
    return True

def _remove_metrics_image_from_local(metrics_image_filepath): # Функция удаляющая файл с графиком с локального диска
    import os
